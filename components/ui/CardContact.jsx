...
export default function CardContact({ item, type = "view", onPress1, onPress2 }) {
    ...
    return (
        <View className="w-full bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 mt-2">
            <View className=" flex-row items-center py-5 px-4">
                {
                    item.foto ? (
                        <Image source={{ uri: Global.API_URL + "/" + item.foto }} style={{ width: 100, height: 100, borderRadius: 200 }} />
                    ) : <Image style={{ width: 100, height: 100, borderRadius: 200 }} source={{ uri: "https://static.vecteezy.com/system/resources/thumbnails/005/129/844/small_2x/profile-user-icon-isolated-on-white-background-eps10-free-vector.jpg" }} />
                }
                <View className="ml-4">
                    ...
                    <View className="flex-row gap-2">
                        {
                            type === "view" ? (
                                <>
                                    <Pressable onPress={onPress1} onPressIn={() => setPress(true)} onPressOut={() => setPress(false)} className={`${press && 'opacity-50'} inline-flex items-center px-4 py-2 bg-primary rounded-lg focus:ring-4 focus:outline-none focus:ring-primary dark:bg-primary dark:focus:ring-primary`}>
                                        <Text className="text-white text-sm font-bold text-center">Ver perfil</Text>
                                    </Pressable>
                                    <Pressable onPress={onPress2} onPressIn={() => setPress2(true)} onPressOut={() => setPress2(false)} className={`${press2 && 'opacity-50 bg-red-500 '} inline-flex items-center px-4 py-2 rounded-lg border border-red-500`}>
